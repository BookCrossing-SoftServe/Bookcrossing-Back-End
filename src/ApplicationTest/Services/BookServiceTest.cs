...
namespace ApplicationTest.Services
{
    ...
    internal class BookServiceTest
    {
        ...
        public async Task GetAllBooks_WhenHasSearchTerm_Returns_books_filtered_by_Book_Title()
        {
            ...
            var query = new BookQueryParams { Page = 1, PageSize = 10, Languages = new[] { 1, 2, 3 }, SearchTerm = "CLR" };
        ...
        private List<Book> GetPopulatedBooks()
        {
            ...
            var list = new List<Book>
            {
                ...
                new Book
                {
                    Id = 3, BookGenre = new List<BookGenre> { genres[3] }, BookAuthor = new List<BookAuthor> { authors[2] },
                    Name = "ICE CLR", Language = languages[2], LanguageId = 3, State = BookState.Available,
                    User = users[0], UserId = 1
                },
                new Book
                {
                    Id = 4, BookGenre = new List<BookGenre> { genres[4] },
                    BookAuthor = new List<BookAuthor> { authors[3], authors[4] }, Name = "FIRE", Language = languages[0],
                    LanguageId = 1, State = BookState.Available, User = users[0], UserId = 1
                }
            };
            return list;
        }
